...
/**
 * Native SQLite database helper that accesses the same database as Flutter's sqflite.
 * This allows Kotlin to read/write sync status independently of Flutter engine.
 */
class NativeSyncDatabase(context: Context) : SQLiteOpenHelper(
    context,
    ...
    fun isFileSynced(filePath: String): Boolean {
        ...
        return cursor.use {
            if (!it.moveToFirst()) {
                ...
                val currentModified = java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS", java.util.Locale.US)
